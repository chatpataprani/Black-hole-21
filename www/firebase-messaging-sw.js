/* Black Hole 21 web FCM service worker. Public Firebase web config only; no private credentials. */
self.addEventListener("notificationclick", (event) => {
  event.notification.close();
  event.waitUntil(
    clients.matchAll({ type: "window", includeUncontrolled: true }).then((clientList) => {
      for (const client of clientList) if ("focus" in client) return client.focus();
      if (clients.openWindow) return clients.openWindow("https://black-hole-21.onrender.com/");
      return undefined;
    })
  );
});
